...
tglitch = [tstart + 0.4 * Tdata, tstart + 0.7 * Tdata]
tglitch /= tref
df_permanent = [5e-2, 5e-2]
df_tau = [5e-2, 5e-2]
...
# for file, i, N in S.write_frame_files(fs=1, Tframe=1800, comment="simCW", out_dir='./sfts/'):
#     print('Generated frame file %s (%i of %i)' % (file, i+1, N))


# Lists for multiple glitches
tglitch = [0, 30 * 86400, 60 * 86400]  # Glitch times
df_permanent = [5e-2, 3e-2, 2e-2]      # Permanent frequency changes
df_tau = [5e-2, 4e-2, 3e-2]            # Transient frequency changes
df1_permanent = [-5e-11, -3e-11, -2e-11]  # Permanent fdot changes
df1_tau = [-5e-11, -4e-11, -3e-11]     # Transient fdot changes
tau = [20 * 86400, 15 * 86400, 10 * 86400]  # Decay timescales
dt_wf = 5
alpha = 6.12
delta = 1.02
detector = 'H1'
